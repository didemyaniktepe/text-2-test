...
class ToDoTestEvaluator:
    ...
    def __init__(self):
        ...
        self.todo_directories = {
            "deepseek": "../ToDo/deep_seek",
            "openai": "../ToDo/open_ai"
        }
    ...
    def discover_test_executions(self) -> List[TestExecutionInfo]:
        ...
        for model, base_path in self.todo_directories.items():
            pattern = f"{base_path}/*/"
    ...
    def create_evaluation_prompt(self, scenario: str, code: str, test_name: str) -> str:
        return f"""# ToDo Test Code Evaluation - Scenario vs Code Alignment

You are a senior Playwright testing engineer. Evaluate whether the following test scenario and the generated code align.
...
"""
    ...
    def evaluate_test_execution(self, test_info: TestExecutionInfo, evaluation_model: str) -> Dict[str, Any]:
        print(f"Evaluating: {test_info.test_name} ({test_info.model}) - {evaluation_model}")
    ...
    def generate_json_report(self, output_file: str = "todo_evaluation_results.json"):
        print("Starting ToDo test evaluation...")
        
        test_executions = self.discover_test_executions()
        print(f"{len(test_executions)} test execution found")
        ...
        print(f"{len(test_executions)} test execution is being processed...")
        
        for i, test_info in enumerate(test_executions, 1):
            print(f"\nProcessing {i}/{len(test_executions)}: {test_info.test_name} ({test_info.model})")
        ...
        print(f"Total evaluation: {len(results['test_evaluations'])}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ToDo Test Evaluator - JSON Format")
    parser.add_argument("--output", "-o", default="todo_evaluation_results.json",
