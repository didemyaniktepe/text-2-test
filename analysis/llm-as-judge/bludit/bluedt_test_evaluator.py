...
class BLUEDTTestEvaluator:
    ...
    def __init__(self):
        ...
        self.bluedt_directories = {
            "deepseek": "BLUEDT/deep_seek",
            "openai": "BLUEDT/open_ai"
        }
    
    def discover_test_executions(self) -> List[TestExecutionInfo]:
        test_executions = []
        
        for model, base_path in self.bluedt_directories.items():
            pattern = f"{base_path}/*/"
    ...
    def create_evaluation_prompt(self, scenario: str, code: str, test_name: str) -> str:
        return f"""# BLUEDT CMS Test Code Evaluation - Scenario vs Code Alignment

You are a senior Playwright testing engineer. Evaluate whether the following test scenario and the generated code align for BLUEDT CMS application.
...
**CRITICAL**: Be objective and consistent. Follow the scoring guide exactly. Pay special attention to scenario-code alignment for BLUEDT CMS operations.
"""

    def evaluate_test_execution(self, test_info: TestExecutionInfo, evaluation_model: str) -> Dict[str, Any]:
        print(f"🔍 Değerlendiriliyor: {test_info.test_name} ({test_info.model}) - {evaluation_model} ile")
    ...
    def generate_json_report(self, output_file: str = "bluedt_evaluation_results.json"):
        print("Starting BLUEDT test evaluation...")
        
        test_executions = self.discover_test_executions()
        print(f"Found {len(test_executions)} test executions")
        ...
        for i, test_info in enumerate(test_executions, 1):
            print(f"Processing {i}/{len(test_executions)}: {test_info.test_name} ({test_info.model})")
            ...
            for eval_model in self.available_models:
                print(f"  Evaluating {test_info.model} test with {eval_model}...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BLUEDT Test Evaluator - JSON Format")
    parser.add_argument("--output", "-o", default="bluedt_evaluation_results.json",
